...
export function ArticlesTable({ articles }: { articles: ArticleWithCategory[] }) {
  return (
    <div className="overflow-hidden rounded-[26px] border border-white/80 bg-[#F3EFE8] shadow-[0_10px_24px_rgba(13,22,25,0.04),inset_0_1px_0_rgba(255,255,255,0.82)]">
          ...
          <tbody className="divide-y divide-white/70">
            {articles.map((article) => (
              <tr key={article.id} className="transition-colors hover:bg-white/45">
                ...
                <td className="px-6 py-4">
                  <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${article.status === "published" ? "border border-[#D7EEDA] bg-[#F4FBF5] text-[#2D7A39]" : "border border-[#F1E0BD] bg-[#FFF9EE] text-[#9A6B15]"}`}>
                    {article.status}
                  </span>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    <Link href={`/${article.category?.slug || "bookkeeping-accounting"}/${article.slug}`} target="_blank" className="p-2 text-[#5A6269] hover:text-[#037aff] hover:bg-[#EBF4FF] rounded-lg transition-colors">
                      <Eye className="w-4 h-4" />
                    </Link>
                    <Link href={`/admin/articles/${article.id}`} className="p-2 text-[#5A6269] hover:text-[#037aff] hover:bg-[#EBF4FF] rounded-lg transition-colors">
      ...
      <div className="flex items-center justify-between border-t border-white/80 bg-white/55 px-6 py-4 text-sm text-[#5A6269]">
        <span>Showing 1 to {articles.length} of {articles.length} entries</span>
        <div className="flex gap-1">
          <button className="rounded-xl border border-[#D9D4CB] bg-white px-3 py-1.5 disabled:opacity-50" disabled>Prev</button>
          <button className="rounded-xl border border-[#D9D4CB] bg-white px-3 py-1.5 disabled:opacity-50" disabled>Next</button>
        </div>
      </div>
    </div>
  );
