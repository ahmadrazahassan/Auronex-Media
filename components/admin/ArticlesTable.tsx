...
export function ArticlesTable({ articles }: { articles: ArticleWithCategory[] }) {
  return (
    <div className="overflow-hidden rounded-[26px] border border-[#E2DFD8] bg-white shadow-[0_10px_24px_rgba(13,22,25,0.04),inset_0_1px_0_rgba(255,255,255,0.9)]">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-[#E2DFD8]">
            {articles.map((article) => (
              <tr key={article.id} className="transition-colors hover:bg-[#FAF9F6]">
                <td className="px-6 py-4">
                  <p className="text-sm font-medium text-[#0d1619] line-clamp-1">{article.title}</p>
                </td>
                <td className="px-6 py-4">
                  <span className="rounded-full border border-[#E2DFD8] bg-white px-2.5 py-1 text-xs font-medium whitespace-nowrap text-[#037aff]">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    <Link href={`/${article.category?.slug || "bookkeeping-accounting"}/${article.slug}`} target="_blank" className="p-2 text-[#5A6269] hover:text-[#037aff] hover:bg-[#FAF9F6] rounded-lg transition-colors">
                      <Eye className="w-4 h-4" />
                    </Link>
                    <Link href={`/admin/articles/${article.id}`} className="p-2 text-[#5A6269] hover:text-[#037aff] hover:bg-[#FAF9F6] rounded-lg transition-colors">
      ...
      <div className="flex items-center justify-between border-t border-[#E2DFD8] bg-white px-6 py-4 text-sm text-[#5A6269]">
        <span>Showing 1 to {articles.length} of {articles.length} entries</span>
        <div className="flex gap-1">
          <button className="rounded-xl border border-[#E2DFD8] bg-white px-3 py-1.5 disabled:opacity-50" disabled>Prev</button>
          <button className="rounded-xl border border-[#E2DFD8] bg-white px-3 py-1.5 disabled:opacity-50" disabled>Next</button>
        </div>
      </div>
    </div>
  );
