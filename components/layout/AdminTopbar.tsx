...
export function AdminTopbar() {
  ...
  return (
    <div className={cn("sticky top-0 z-40 w-full px-4 pt-4 transition-transform duration-300 md:px-6 md:pt-6", hidden ? "-translate-y-[150%]" : "translate-y-0")}>
      <header className="mx-auto flex w-full max-w-5xl items-center justify-between gap-2 rounded-[24px] border border-[#E2DFD8] bg-white/80 p-2 shadow-[0_8px_32px_rgba(13,22,25,0.05),inset_0_1px_0_rgba(255,255,255,1)] backdrop-blur-xl md:w-max md:gap-4 md:px-3 md:py-2.5">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active
                    ? "bg-[#0d1619] text-white shadow-md"
                    : "text-[#5A6269] hover:bg-[#F0EFEB] hover:text-[#0d1619]"
                )}
              >
                {link.name}
              </Link>
        ...
        <div className="hidden items-center gap-1 sm:flex">
          <Link href="/" target="_blank" className="rounded-xl px-3 py-2 text-[14px] font-medium text-[#5A6269] transition-all hover:bg-[#F0EFEB] hover:text-[#0d1619] active:scale-95">
            Site
          </Link>
          <button
            type="button"
            onClick={handleSignOut}
            className="flex items-center justify-center rounded-xl p-2 text-[#5A6269] transition-all hover:bg-[#F0EFEB] hover:text-[#0d1619] active:scale-95"
            aria-label="Sign out"
          >
